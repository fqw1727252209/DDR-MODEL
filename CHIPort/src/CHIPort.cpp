...
make_read_data_phase(const ARM::CHI::Phase &fw_phase,
                     const ARM::CHI::DatOpcode dat_opcode) {
...
void CHIPort::dfi_clock_posedge() {
  wdat_push_s2();
  wdat_decode_s1();
...
void CHIPort::noc_clock_posedge() {
  rdat_arbit_s1();
  resp_arbit_s1();
}
...
void CHIPort::noc_clock_negedge() {
  for (const auto channel : {
           ARM::CHI::CHANNEL_REQ,
           ARM::CHI::CHANNEL_RSP,
           ARM::CHI::CHANNEL_DAT,
       }) {
    channels[channel].send_flits(
        channel, [this](ARM::CHI::Payload &payload, ARM::CHI::Phase &phase) {
          return target.nb_transport_bw(payload, phase);
        });
  }
}

tlm::tlm_sync_enum CHIPort::nb_transport_fw(ARM::CHI::Payload &payload,
                                            ARM::CHI::Phase &phase) {
...
tlm::tlm_sync_enum CHIPort::nb_transport_bw(tlm::tlm_generic_payload &payload,
                                            tlm::tlm_phase &phase,
...
void CHIPort::peqCallback(tlm::tlm_generic_payload &payload,
                          const tlm::tlm_phase &phase) {
  if (phase == UIF_CREDIT) {
    UifSideBandInfo uifSideBandInfo =
        payload.get_extension<UifSideBandExtension>()->_uif_side_band_info;
    ...
    // unsigned cmd_id =
    // payload.get_extension<UifExtension>()->_uif_info.cmd_id;
    DPRINT_INFO(true, "CHI Port", "Get the Rdat transaction First Data");
  } else if (phase == UIF_RDAT_END) {
    // 将rdat buffer中的数据标定为读完成，同时将payload中的数据copy到对应的CHI
    // Flit
    ...
    // 输出读事务的完成时间，并将指针插入到对应的队列map中，等待rdata_info
    // 被移除时，记录对应的读数据在CHI接口处的输出时间
  } else if (phase == UIF_WDAT_REQ) {
    // unsigned wr_cmd_id =
    // payload.get_extension<UifExtension>()->_uif_info.cmd_id; 当拍传递wdat
    // Begin,并且根据写请求的size，决定传递的拍数，如果为2，则在下一个周期传递UIF_WDAT_END，如果为1，在在delta时间后，传递UIF_WDAT_END
    tlm::tlm_phase wdat_begin_phase = UIF_WDAT_BEGIN;
    sc_core::sc_time delay = sc_core::SC_ZERO_TIME;
    iSocket->nb_transport_fw(payload, wdat_begin_phase, delay);
    if (payload.get_data_length() <=
        32) // TODO: 32 is fixed value,应该是由uif接口的位宽 计算得出
    {
...
CHIPort::CHIPort(const sc_core::sc_module_name &name,
                 const Configure &configure, unsigned data_width_bits,
                 const sc_core::sc_time &clock_period)
    : sc_core::sc_module(name), _configure(configure),
      data_width_bytes(data_width_bits / 8),
      target("target", *this, &CHIPort::nb_transport_fw,
             ARM::TLM::PROTOCOL_CHI_E, data_width_bits),
      dfi_clock("dfi_clcok"), noc_clock("noc_clock"),
      clock_period(clock_period),
      payloadEventQueue(this, &CHIPort::peqCallback), memoryManager(false) {

  wdataBufferArray =
      std::make_unique<WdataBufferArray>(_configure, data_width_bytes);
  rdDataInfo = std::make_unique<RdDataInfo>(_configure, data_width_bytes);
  p2cFifo = std::make_unique<P2cFifo>(_configure, *wdataBufferArray.get(),
                                      *rdDataInfo.get(), clock_period);
  responseQueues = std::make_unique<ResponseQueues>(_configure);
  retryResourceManager = std::make_unique<RetryResourceManager>(
      _configure, *p2cFifo.get(), *wdataBufferArray.get(), clock_period);
...
void CHIPort::req_decode_s1() {
  if (!channels[ARM::CHI::CHANNEL_REQ].rx_queue.empty()) {
    if (!responseQueues->IsResponseQueueFull(ResponseQueueType::RetryAck)) {
      ...
      // DPRINT_INFO(true, "CHIPort", "Request Channel stage 1, s1 queue size:
      // %d",req_s1.size());
    }
  }
}

void CHIPort::req_decision_s2() {
  if (!req_s1.empty()) {
    CHIFlit &req_flit = req_s1.front();
    req_s2.push_back(std::move(req_flit));
    req_s1.pop_front();
    // DPRINT_INFO(true, "CHIPort", "Request Channel stage 2, s2 queue size:
    // %ld",req_s2.size());
    ...
    if (req_flit.phase.req_opcode == ARM::CHI::REQ_OPCODE_WRITE_NO_SNP_FULL ||
      ...
      if (!req_accepted) {
        ...
        tlm::tlm_generic_payload &trans =
            memoryManager.allocate(req_flit, false);
        p2cFifo->InsertTpwRequest(
            req_flit, static_cast<tlm::tlm_generic_payload *>(&trans),
            allocated_dbid, qos.GetQosLevel());
      }
    } else if (req_flit.phase.req_opcode == ARM::CHI::REQ_OPCODE_READ_NO_SNP ||
               req_flit.phase.req_opcode ==
                   ARM::CHI::REQ_OPCODE_READ_NO_SNP_SEP) {
      ...
      if (!req_accepted) {
        ...
        if (qos.GetQosLevel() == PriorityClass::HPR) {
          // 分配tlm::tlm_generic_payload
          tlm::tlm_generic_payload &trans =
              memoryManager.allocate(req_flit, true);
          // 插入到HPR队列
          p2cFifo->InsertHprRequest(
              req_flit, static_cast<tlm::tlm_generic_payload *>(&trans),
              qos.GetQosLevel());
        } else if (qos.GetQosLevel() == PriorityClass::LPR ||
                   qos.GetQosLevel() == PriorityClass::GPR) {
          // 分配tlm::tlm_generic_payload
          tlm::tlm_generic_payload &trans =
              memoryManager.allocate(req_flit, true);
          // 插入到LPR队列
          p2cFifo->InsertLGprRequest(
              req_flit, static_cast<tlm::tlm_generic_payload *>(&trans),
              qos.GetQosLevel());
        }
      }
    ...
    } else if (req_flit.phase.req_opcode ==
               ARM::CHI::REQ_OPCODE_CLEAN_SHARED_PERSIST) {
...
void CHIPort::req_pop_s3() {
  if (!req_s2.empty())
    req_s2.pop_front();
  if (!p2cFifo->IsQueueEmpty()) {
    // DPRINT_INFO(true, "CHIPort", "Request Channel stage 3, s3 queue empty:
    // %d",p2cFifo->IsQueueEmpty());
    ...
    if (winning_queue == QueueType::HPR || winning_queue == QueueType::LPR) {
      ...
      CHIFlit req_flit =
          CHIFlit(rd_front_request.payload, rd_front_request.phase);
      ...
      // sent to downstream
      SendUifRequest(p2cFifo->GetRdFrontRequest(winning_queue),
                     allocated_rdata_id, true);
      ...
      responseQueues->InsertCompResp(CHIFlit(wr_front_request.second.payload,
                                             wr_front_request.second.phase));
...
bool CHIPort::handle_WriteNoSnp(const CHIFlit &flit, PriorityClass qos_level) {
  // return false;
  if (!flit.phase.allow_retry) {
    return true;
  } else {
    if (qos_level == PriorityClass::TPW || qos_level == PriorityClass::GPW) {
      if (retryResourceManager->has_retry_cmd(PriorityClass::TPW) ||
          retryResourceManager->has_retry_cmd(PriorityClass::GPW))
        return false;
      else { //
        if (retryResourceManager
                ->is_tpw_full()) // wcq full(pre-allocated + already occupied)
                                 // || wdatabuffer full
          return false;
        else
          return true;
      ...
      SC_REPORT_ERROR("Handle Write Transaction Retry Decision",
                      "Invalid Priority Class: not Tpw or Gpw");
    }
  }
  return false;
}

bool CHIPort::handle_ReadNoSnp(const CHIFlit &flit, PriorityClass qos_level) {
  if (!flit.phase.allow_retry) {
    return true;
  } else {
    ...
    if (qos_level == PriorityClass::HPR) {
      if (retryResourceManager->has_retry_cmd(PriorityClass::HPR))
        return false;
      else if (retryResourceManager
                   ->is_gpr_expired_and_rd_queue_only_one_space()) // the read
                                                                   // queue has
                                                                   // only one
                                                                   // buffer
                                                                   // space, and
        ...
        if (retryResourceManager
                ->is_hpr_full()) // hpr full(pre-allocated + already occupied)
          return false;
        else
          return true;
      }
      ...
      if (retryResourceManager->has_retry_cmd(PriorityClass::LPR))
        ...
        if (retryResourceManager
                ->is_lgpr_full()) // lpr full(pre-allocated + already occupied)
          return false;
        else
          return true;
      }
      ...
      if (retryResourceManager->has_retry_cmd(PriorityClass::GPR))
        ...
        if (retryResourceManager
                ->is_lgpr_full()) // gpr full(pre-allocated + already occupied)
          return false;
        else
          return true;
      }
...
bool CHIPort::handle_CleanShardPersist(const CHIFlit &flit) {
  if (!flit.phase.allow_retry) {
    return true;
  } else {
    if (!retryResourceManager->is_type_empty(RetryType::CMO)) {
      ...
      if (retryResourceManager
              ->is_cmo_full()) // cmo full(pre-allocated + already occupied)
        return false;
      else
        return true;
    }
...
bool CHIPort::handle_PrefetchTgt(const CHIFlit &flit) {
  if (!flit.phase.allow_retry) {
    return true;
  } else {
  }
...
void CHIPort::resp_arbit_s1() {
  if (responseQueues->HasRspPending()) {
    ...
    channels[ARM::CHI::CHANNEL_RSP].tx_queue.emplace_back(
        std::move(responseQueues->GetQueueFront(
            static_cast<uint8_t>(winning_queue_index))));
    responseQueues->QueuePop(static_cast<uint8_t>(winning_queue_index));
  }
}

void CHIPort::resp_gen_pcrd() {
  if (retryResourceManager->is_need_to_send_pcrd_grant()) {
    PortCmdType pcrd_cmd_type = retryResourceManager->select_type_cmd_type();
    unsigned pcrd_tgt_id =
        retryResourceManager->select_src_id(pcrd_cmd_type); // when do t
    ...
    if (pcrd_cmd_type == PortCmdType::HPR) {
      pcrd_phase.pcrd_type = 2 & 0xF;
    } else if (pcrd_cmd_type == PortCmdType::GPR) {
      pcrd_phase.pcrd_type = 1 & 0xF;
    } else if (pcrd_cmd_type == PortCmdType::LPR) {
      pcrd_phase.pcrd_type = 0 & 0xF;
    } else if (pcrd_cmd_type == PortCmdType::TPW) {
      pcrd_phase.pcrd_type = 3 & 0xF;
    } else if (pcrd_cmd_type == PortCmdType::GPW) {
      pcrd_phase.pcrd_type = 4 & 0xF;
    } else if (pcrd_cmd_type == PortCmdType::CMO) {
      pcrd_phase.pcrd_type = 15 & 0xF;
    } else {
      pcrd_phase.pcrd_type = 0 & 0xF;
    }

    pcrd_phase.rsp_opcode = ARM::CHI::RSP_OPCODE_PCRD_GRANT;
    pcrd_phase.c_busy = 0x7 & 3; // PcrdGrant set cbusy field 3'b011
...
/*Read Data Channel*/
void CHIPort::rdat_arbit_s1() {
  if (rdDataInfo->has_entry_ready()) {
    if (rdDataInfo->has_entry_sending_data()) {
      auto [element_id, element_entry, next_data_id] =
          rdDataInfo->get_sending_data_entry();
      bool is_rdnosnpsep = element_entry.phase.req_opcode ==
                           ARM::CHI::REQ_OPCODE_READ_NO_SNP_SEP;
      ARM::CHI::DatOpcode dat_opcode = is_rdnosnpsep
                                           ? ARM::CHI::DAT_OPCODE_DATA_SEP_RESP
                                           : ARM::CHI::DAT_OPCODE_COMP_DATA;
      ARM::CHI::Phase data_phase =
          make_read_data_phase(element_entry.phase, dat_opcode);
      data_phase.data_id = next_data_id;
      channels[ARM::CHI::CHANNEL_DAT].tx_queue.emplace_back(
          std::move(CHIFlit(element_entry.payload, data_phase)));
      if (rdDataInfo->is_entry_data_finished(element_id)) {
        rdDataInfo->erase_entry(element_id);
      }
    } else {
      auto [element_id, element_entry, next_data_id] =
          rdDataInfo->get_ready_entry();
      bool is_rdnosnpsep = element_entry.phase.req_opcode ==
                           ARM::CHI::REQ_OPCODE_READ_NO_SNP_SEP;
      ARM::CHI::DatOpcode dat_opcode = is_rdnosnpsep
                                           ? ARM::CHI::DAT_OPCODE_DATA_SEP_RESP
...
/*UIF send Request*/
void CHIPort::SendUifRequest(const QueueEntry &entry, unsigned cmd_id,
                             bool is_rd) {
  ...
  uif_info.cmd_type =
      is_rd ? CmdType::RD : (uif_info.is_rmw ? CmdType::RMW : CmdType::WR);
