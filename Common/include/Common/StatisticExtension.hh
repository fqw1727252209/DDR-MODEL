...
public:
  ...
  inline void RecordInPortTime(sc_core::sc_time record_time) {
    m_entering_port_time = record_time;
  }
  inline void RecordOutPortTime(sc_core::sc_time record_time) {
    m_leaving_port_time = record_time;
  }

  inline void RecordDecodedAddress(DecodedAddress decoded_address) {
    m_decoded_address = decoded_address;
  }

  inline void RecordInCamTime(sc_core::sc_time record_time) {
    m_entering_cam_time = record_time;
  }
  inline void RecordOutCamTime(sc_core::sc_time record_time) {
    m_leaveing_cam_time = record_time;
  }

  inline void RecordCmdTime(sc_core::sc_time record_time, DramCommand cmd) {
    m_cmd_time_vec.emplace_back(record_time, cmd);
  } // need to record cmd type?
  ...
  inline const std::vector<std::pair<sc_core::sc_time, DramCommand>> &
  GetCmdTimeVec() const {
    return m_cmd_time_vec;
  }

  // 新增数据传输相关时间的getter和setter
  inline sc_core::sc_time GetUiFDataBeginTime() const {
    return m_uif_data_begin_time;
  }
  inline sc_core::sc_time GetUiFDataEndTime() const {
    return m_uif_data_end_time;
  }
  inline void RecordUiFDataBeginTime(sc_core::sc_time time) {
    m_uif_data_begin_time = time;
  }
  inline void RecordUiFDataEndTime(sc_core::sc_time time) {
    m_uif_data_end_time = time;
  }

  inline sc_core::sc_time GetDfiDataBeginTime() const {
    return m_dfi_data_begin_time;
  }
  inline sc_core::sc_time GetDfiDataEndTime() const {
    return m_dfi_data_end_time;
  }
  inline void RecordDfiDataBeginTime(sc_core::sc_time time) {
    m_dfi_data_begin_time = time;
  }
  inline void RecordDfiDataEndTime(sc_core::sc_time time) {
    m_dfi_data_end_time = time;
  }

  inline sc_core::sc_time GetDqDataBeginTime() const {
    return m_dq_data_begin_time;
  }
  inline sc_core::sc_time GetDqDataEndTime() const {
    return m_dq_data_end_time;
  }
  inline void RecordDqDataBeginTime(sc_core::sc_time time) {
    m_dq_data_begin_time = time;
  }
  inline void RecordDqDataEndTime(sc_core::sc_time time) {
    m_dq_data_end_time = time;
  }
  inline int GetTransactionId() const { return transaction_id; }
  inline const DecodedAddress &GetDecodedAddress() const {
    return m_decoded_address;
  }
  ...
  // 使用logger输出统计信息
  void PrintStatisticsToLogger(
      const std::string &logger_name = "default_logger") const;
  void PrintStatisticsToLogger(Logger &logger) const;
...
private:
  ...
  std::vector<std::pair<sc_core::sc_time, DramCommand>>
      m_cmd_time_vec; // record this cmd do the ddr cmd sending
