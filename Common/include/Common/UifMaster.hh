...
class UifMaster: public sc_core::sc_module
{
    ...
    public:
        UifMaster(const sc_core::sc_module_name& name, TrafficType traffic_type,const dmu::Configure& _config)
            : sc_module(name)
        ...
        void bind(Controller::MemoryController& mc)
        {
            iSocket.bind(mc.tSocket);
        }

        bool SendUifTrans(uint64_t addr, CmdType cmd_type, uint8_t qos_value,unsigned trans_id)
        {
            ...
            {
                DPRINT_INFO(TOP_DEBUG,name()," trans is successfully sent to downstream");
                return true;
            }
        ...
        void SendMultiTrans()
        {
            while(true)
            {
                ...
                {
                    DPRINT_INFO(false, name()," State Is: %d , winning queue is: %d", pa_arbiter.state,QueueType::LPR);
                    SendLprTrans();
                    DPRINT_INFO(false, name(),"Has Lpr Cmd Pending: %ld", lpr_queue.GetQueueSize());
                }
                else if(pa_arbiter.GetWinningQueue() == QueueType::HPR)
                {
                    DPRINT_INFO(false, name()," State Is: %d , winning queue is: %d", pa_arbiter.state,QueueType::HPR);

                    SendHprTrans();
                    DPRINT_INFO(false, name(),"Has Hpr Cmd Pending: %ld", hpr_queue.GetQueueSize());
                }
                else if(pa_arbiter.GetWinningQueue() == QueueType::TPW)
                {
                    DPRINT_INFO(false, name()," State Is: %d , winning queue is: %d", pa_arbiter.state,QueueType::TPW);
                ...
                if(!stage_queue3.empty())
                {
                    ...
                    if(trans->get_extension<UifExtension>()->GetQosLevel() == PriorityClass::HPR)
                    {
                        hpr_queue.InsertRequest(trans);
                        DPRINT_INFO(TOP_DEBUG,name(),"hpr trans is successfully Added, Queue Size: %ld",hpr_queue.GetQueueSize());
                    }
                    else if(trans->get_extension<UifExtension>()->GetQosLevel() == PriorityClass::LPR ||
                            trans->get_extension<UifExtension>()->GetQosLevel() == PriorityClass::GPR)
                    {
                        lpr_queue.InsertRequest(trans);
                        DPRINT_INFO(TOP_DEBUG,name(),"lpr trans is successfully Added, Queue Size: %ld",lpr_queue.GetQueueSize());
                    }
                    else if(trans->get_extension<UifExtension>()->GetQosLevel() == PriorityClass::TPW ||
                            trans->get_extension<UifExtension>()->GetQosLevel() == PriorityClass::GPW)
                    {
                        tpw_queue.InsertRequest(trans);
                        DPRINT_INFO(TOP_DEBUG,name(),"tpw trans is successfully Added, Queue Size: %ld",tpw_queue.GetQueueSize());
                    }
                    else
                    {
        ...
        void TrafficGenerator()
        {
            ...
            // constexpr uint64_t base_addr0{0x8000'0000};
            constexpr uint64_t base_addr0{0x0000'0000};
            constexpr uint64_t high_addr0{0xffff'ffff};
            // constexpr uint64_t base_addr1{0x1000'8000'0000};
            constexpr uint64_t base_addr1{0x0000'0000'0000};
            constexpr uint64_t high_addr1{0xffff'ffff'ffff};
            ...
            // unsigned trace_length{16384};// 1048576 <-> 64MB ,524288 <-> 32MB, 4096 <-> 1MB, 8192 <-> 2MB, 16384 <-> 4MB, 32768 <-> 8MB
            ...
            //     else
            //         cmd_type = CmdType::RD;
            //     AddTrans(base_addr0 + stride * (i), cmd_type, 15, trans_id);
            //     trans_id++;
            //     wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
            //     i++;
            // }
            ...
            {
                unsigned trace_length{10000}; // stream rd
                while(i < trace_length)
                {

                    AddTrans(base_addr0 + stride * (i), CmdType::RD, 15, trans_id);
                    trans_id++;
                    wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
                    i++;
                }
            ...
            {
                unsigned trace_length{10000}; // stream wr
                while(i < trace_length)
                {

                    AddTrans(base_addr0 + stride * (i), CmdType::WR, 15, trans_id);
                    trans_id++;
                    wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
                    i++;
                }
            ...
            {
                unsigned trace_length{10000};   // random RD
                while(i < trace_length)
                {
                    AddTrans(random_addr_gen.Generate(base_addr0, base_addr0 + 0x2000'0000), CmdType::RD, 15, trans_id);
                    trans_id++;
                    wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
                    i++;
                }
            ...
            {
                unsigned trace_length{10000};   // random WR
                while(i < trace_length)
                {
                    AddTrans(random_addr_gen.Generate(base_addr0, base_addr0 + 0x2000'0000), CmdType::WR, 15, trans_id);
                    trans_id++;
                    wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
                    i++;
                }
            ...
            {
                unsigned trace_length{10240}; // rand copy
                while (i < trace_length) {
                    AddTrans(random_addr_gen.Generate(base_addr0, base_addr0+0x2000'0000), CmdType::WR, 15, trans_id);
                    trans_id++;
                    AddTrans(random_addr_gen.Generate(base_addr0+0x4000'0000,base_addr0+0x6000'0000), CmdType::RD, 15, trans_id);
                    trans_id++;
                    wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
                    i++;
                }
            ...
            {
                unsigned trace_length{10240}; // rand add
                while(i < trace_length)
                {
                    AddTrans(random_addr_gen.Generate(base_addr0,base_addr0+0x2000'0000), CmdType::WR, 15, trans_id);
                    trans_id++;
                    AddTrans(random_addr_gen.Generate(base_addr0+0x4000'0000, base_addr0+0x6000'0000), CmdType::RD, 15, trans_id);
                    trans_id++;
                    AddTrans(random_addr_gen.Generate(base_addr0+0x8000'0000, base_addr0+0xA000'0000), CmdType::RD, 15, trans_id);
                    trans_id++;
                    wait(1 *sc_core::sc_time(config.mem_spec->tCK_mc));
                    i++;
                }
        ...
        void AddTrans(uint64_t addr, CmdType cmd_type, uint8_t qos_value,unsigned trans_id)
        {
            tlm::tlm_generic_payload& trans = mm.allocate();
            ...
            if(cmd_type == CmdType::RD)
            {
                ...
                //     DPRINT_INFO(TOP_DEBUG,name(),"Hpr trans is successfully Added, Queue Size: %ld",hpr_queue.GetQueueSize());
                // }
                // else if(_uif_info.qos.GetQosLevel() == PriorityClass::LPR || _uif_info.qos.GetQosLevel() == PriorityClass::GPR)
                // {
    ...
    private:
        tlm::tlm_sync_enum nb_transport_bw(tlm::tlm_generic_payload& trans, tlm::tlm_phase& phase, sc_core::sc_time& delay)
        {
            ...
            {
                DPRINT_INFO(false, "Uif Dat Channel", "Transmit Data END");
            }
            else if(phase == UIF_RDAT_BEGIN)
            {
                DPRINT_INFO(false, "Uif Dat Channel", "Transmit Data Begin");
            }
            else if(phase == UIF_CREDIT)
            {
                auto uif_sideband_info = trans.get_extension<dmu::UifSideBandExtension>()->_uif_side_band_info;
                if(uif_sideband_info.lpr_credit_valid)
                {
                    DPRINT_INFO(true, "Uif Credit Channel", "Receive Lpr Credit");
                    lpr_queue.ReceiveCredit();
                }
                if(uif_sideband_info.hpr_credit_valid)
                {
                    DPRINT_INFO(true, "Uif Credit Channel", "Receive Hpr Credit");
                    hpr_queue.ReceiveCredit();
                }
                if(uif_sideband_info.tpw_credit_valid)
                {
                    DPRINT_INFO(true, "Uif Credit Channel", "Receive Tpw Credit");
            ...
            else if(phase == UIF_WDAT_REQ)
            {
                ...
                {
                    DPRINT_INFO(false, "Uif Write Dat Channel", "Receive Wdat Request");
                    ...
                    sc_core::sc_time delay2{config.mem_spec->tCK_mc+config.mem_spec->tCK_mc};
                    iSocket->nb_transport_fw(trans, phase2, delay2);
                    DPRINT_INFO(false, "Uif Write Dat Channel", "End Wdat Request");
                }
            }
    ...
    private:
        ...
        class RandomAddressGenerator{
            private:
                std::mt19937 gen;
            public:
                explicit RandomAddressGenerator(uint64_t seed = std::random_device{}()):gen(seed){}
        ...
        class RequestQueue{
            ...
            public:
                ...
                bool IsQueueExpired() const { return aging_expired_time >= sc_core::sc_time_stamp(); }
                ...
                void InsertRequest(tlm::tlm_generic_payload* trans) { request_queue.push_back(trans); }
                ...
                // if th command type is rmw, and the lpr has no credit, then the command will be masked
        ...
        void SendLprTrans()
        {
            ...
            if(callback == tlm::tlm_sync_enum::TLM_ACCEPTED)
            {
                DPRINT_INFO(TOP_DEBUG,name(),"lpr trans is successfully sent to downstream");
            ...
            {
                DPRINT_WARNING(TOP_DEBUG,name()," lpr Should send trans, but not accepted for addr collision");
        ...
        void SendHprTrans()
        {
            ...
            if(callback == tlm::tlm_sync_enum::TLM_ACCEPTED)
            {
                DPRINT_INFO(TOP_DEBUG,name(),"hpr trans is successfully sent to downstream");
            ...
            {
                DPRINT_WARNING(TOP_DEBUG,name()," hpr Should send trans, but not accepted for addr collision");
        ...
        void SendTpwTrans()
        {
            ...
            if(callback == tlm::tlm_sync_enum::TLM_ACCEPTED)
            {
                DPRINT_INFO(TOP_DEBUG,name(),"tpw trans is successfully sent to downstream");
            ...
            {
                DPRINT_WARNING(TOP_DEBUG,name()," Tpw Should send trans, but not accepted for addr collision");
        ...
        class PaArbiter
        {
            public:
                ...
                explicit PaArbiter(const RequestQueue& lpr_queue, const RequestQueue& hpr_queue, const RequestQueue& tpw_queue):
            ...
            private:
                ...
                bool IsRdHold() {
                    if(switch_fast)
                    {
                        return (!lpr_queue.IsQueueEmpty() && lpr_queue.HasCredit()) || (!hpr_queue.IsQueueEmpty() && hpr_queue.HasCredit());
                    }
                    else {
                ...
                bool IsHprHighPriority(){
                    return hpr_queue.IsQueueExpired() && hpr_queue.HasCredit() && !hpr_queue.IsQueueEmpty();
                }
                bool IsLprHighPriority(){
                    return (lpr_queue.IsQueueExpired() || lpr_queue.HasExpiredCmd()) && lpr_queue.HasCredit() && !lpr_queue.IsQueueEmpty() 
                            ;
                }
                bool IsTpwHighPriority(){
                    return (tpw_queue.IsQueueExpired() || tpw_queue.HasExpiredCmd()) && tpw_queue.HasCredit() && !tpw_queue.IsQueueEmpty() 
                            ;
                }
                ...
                void UpdateState() {
                    switch (state) {
                        ...
                        case WR:
                            if (IsWrHighPriority())
                                state = WR;
                            else if ((IsRdHighPriority()) || (!hpr_queue.IsQueueEmpty() && hpr_queue.HasCredit()))
                                state = RD;
                            else if (IsWrHold())
                                state = WR;
        ...
        // else if(rd queue has high priority and credit || hpr has req and credit) --> switch 2 RD
        // else if(wr hold) --> keep WR
        // else if(lpr queue has req and credit) --> switch 2 RD
        // else --> switch 2 IDLE
