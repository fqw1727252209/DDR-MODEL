...
private:
  ...
  bool is_blocked; // if RAA counter exceed the RAAMax, then block bank sending
                   // ACT cmd

  uint32_t act_counter{0};
  const uint32_t raa_threshold;  // = RAA_THRESHOLD
  const uint32_t raa_imt;        // RAAIMT: 初级预警阈值，超过则建议发 RFM
  const uint32_t raa_mmt;        // RAAMMT: 最大阈值，超过则强制阻断 ACT
  const uint32_t raa_mult;       // RAAMULT: 每笔 ACT 累加的权重
  bool rfm_req{false};
  bool act_hard_blocked{false};  // RAAMMT 触发，强制阻断 ACT
  ...
  enum BankState {
    ...
    Precharged
  } current_state{Precharged},
      next_state{Precharged};

  bool is_allocated; // whether the bank slice is allocated to manage the
                     // related bank
  ...
  sc_core::sc_time act_tRCD_end_time{
      sc_core::SC_ZERO_TIME}; // when send act cmd, then do mask with ntt update
...
public:
  BankSlice(const Scheduler &scheduler, const Configure &config,
            BSC_INDEX bsc_index)
      : _rd_cam(scheduler.GetRdCam()), _wr_cam(scheduler.GetWrCam()),
        m_bsc_index(bsc_index), tRCD_mc(config.mem_spec->tRCD_mc),
        tRAS_max_mc(config.mem_spec->tCK_mc * 251),
        raa_threshold(config.controller_config->RAA_THRESHOLD),
        raa_imt(config.controller_config->RAAIMT),
        raa_mmt(config.controller_config->RAAMMT),
        raa_mult(config.controller_config->RAAMULT > 0 ? config.controller_config->RAAMULT : 1) {}
  ~BankSlice() = default;

  inline bool IsNeedForcePre() const {
    return sc_core::sc_time_stamp() > act_tRAS_end_time;
  }
  ...
  inline bool IsRfmReq() const { return rfm_req; }
  inline bool IsActBlocked() const { return act_hard_blocked; } // RAAMMT 超限，强制阻断 ACT
  inline void ClearRfmReq() {
    rfm_req = false;
    act_counter = 0;
    act_hard_blocked = false;
  }

  // RTL 对齐：普通 Refresh 发出后，对 act_counter 按 RAADEC 进行退火减少
  inline void DecreaseRaa(unsigned raadec) {
    if (raadec == 0) return;
    if (act_counter > raadec)
      act_counter -= raadec;
    else
      act_counter = 0;
    // 若退火后计数器降回 RAAIMT 以下，解除软预警提示
    if (rfm_req && raa_imt > 0 && act_counter < raa_imt) {
      rfm_req = false;
      std::cout << "@" << sc_core::sc_time_stamp() << ": [RFM] DecreaseRaa: act_counter=" << act_counter
                << " < RAAIMT=" << raa_imt << ", rfm_req cleared (annealing)" << std::endl;
    }
    // RAAMMT 阈值退火后解除 ACT 硬阵断
    if (act_hard_blocked && raa_mmt > 0 && act_counter < raa_mmt) {
      act_hard_blocked = false;
      std::cout << "@" << sc_core::sc_time_stamp() << ": [RFM] DecreaseRaa: act_counter=" << act_counter
                << " < RAAMMT=" << raa_mmt << ", ACT hard-block released" << std::endl;
    }
  }
  ...
  inline bool IsActiving() const {
    return sc_core::sc_time_stamp() < act_tRCD_end_time;
  } // to show whether the BSC bank is in activing time
  ...
  inline sc_core::sc_time &GetRdCmdAvailTime() {
    return next_rd_command_avail_time;
  }
  inline sc_core::sc_time &GetWrCmdAvailTime() {
    return next_wr_command_avail_time;
  }
  ...
  inline bool IsWrCmdAvail() const {
    if (candidate_wr_cmd.is_valid && next_wr_command != Command::NOP &&
        next_wr_command_avail_time <= sc_core::sc_time_stamp()) {
      return true;
    }
    return false;
  }
  inline bool IsRdCmdAvail() const {
    if (candidate_rd_cmd.is_valid && next_rd_command != Command::NOP &&
        next_rd_command_avail_time <= sc_core::sc_time_stamp()) {
      return true;
    }
    return false;
  }

  inline bool IsWrRowCmdAvail() const {
    // RTL 对齐：RAAMMT 触发 act_hard_blocked 时，禁止该 Bank 发出新 ACT 命令
    bool is_act_allowed = !(next_wr_command == Command::ACT && act_hard_blocked);
    return candidate_wr_cmd.is_valid &&
           (next_wr_command == Command::PRE ||
            next_wr_command == Command::ACT) &&
           next_wr_command_avail_time <= sc_core::sc_time_stamp() &&
           is_act_allowed;
  }
  inline bool IsRdRowCmdAvail() const {
    // RTL 对齐：RAAMMT 触发 act_hard_blocked 时，禁止该 Bank 发出新 ACT 命令
    bool is_act_allowed = !(next_rd_command == Command::ACT && act_hard_blocked);
    return candidate_rd_cmd.is_valid &&
  ...
  inline bool IsWrColCmdAvail() const {
    return candidate_wr_cmd.is_valid &&
           (next_wr_command == Command::WR ||
            next_wr_command == Command::WRA) &&
           next_wr_command_avail_time <= sc_core::sc_time_stamp();
  }
  inline bool IsRdColCmdAvail() const {
    return candidate_rd_cmd.is_valid &&
  ...
  inline bool IsRdPageClosePending() const {
    return IsRdCmdAvail() && next_rd_command == Command::PRE;
  } // Bank Rd direction being PRE
  inline bool IsWrPageClosePending() const {
    return IsWrCmdAvail() && next_wr_command == Command::PRE;
  } // Bank Wr direction being PRE

  inline bool IsRdPageOpenPending() const {
    return IsRdCmdAvail() && next_rd_command == Command::ACT;
  } // Bank Rd direction being ACT
  inline bool IsWrPageOpenPending() const {
    return IsWrCmdAvail() && next_wr_command == Command::ACT;
  } // Bank Wr direction being ACT
  ...
  inline bool IsWrNttPageHit() const {
    return candidate_wr_cmd.is_valid &&
           _wr_cam->GetCamEntry(candidate_wr_cmd.cam_index)->IsPageHit();
  }
  inline bool IsRdNttPageHit() const {
    return candidate_rd_cmd.is_valid &&
           _rd_cam->GetCamEntry(candidate_rd_cmd.cam_index)->IsPageHit();
  }

  inline bool IsRdNttExpired() const {
    return candidate_rd_cmd.is_valid &&
           _rd_cam->GetCamEntry(candidate_rd_cmd.cam_index)->IsExpired();
  }
  inline bool IsWrNttExpired() const {
    return candidate_wr_cmd.is_valid &&
           _wr_cam->GetCamEntry(candidate_wr_cmd.cam_index)->IsExpired();
  }
  ...
  void print() {
    std::cout << "Bank Slice: " << m_bsc_index << " ba_addr: " << _ba_addr
              << " is_refresh_waiting: " << is_refresh_waiting
              << " page open: " << page_info.is_open
              << " page open row: " << page_info.open_page << ";\t"
              << "wr ntt valid: " << candidate_wr_cmd.is_valid
              << ", wr ntt cam index: " << candidate_wr_cmd.cam_index
              << "wr cmd: " << Command(next_wr_command).to_string()
              << ", wr cmd avail time: " << next_wr_command_avail_time << "; \t"
              << "rd ntt valid: " << candidate_rd_cmd.is_valid
              << ", rd ntt cam index: " << candidate_rd_cmd.cam_index
              << "rd cmd: " << Command(next_rd_command).to_string()
              << ", rd cmd avail time: " << next_rd_command_avail_time << "; \t"
              << std::endl;
  }
};
} // namespace Controller
