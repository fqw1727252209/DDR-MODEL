...
Scheduler::UpdateRdNttPip(BSC_INDEX bsc_index, RealBaIndex ba_addr, UpdateType update_type)
{
    ...
    if(update_type == UpdateType::NewCmdStore && !rd_cam->IsBaOrderListEmpty(ba_addr)&& rd_cam->IsBaListAvail(ba_addr))
    {
        ...
        DPRINT_ASSERT(rd_cam->GetCamEntry(updated_cam_index)->sdram_addr.real_ba == ba_addr,"Rd Ntt Update:", "ba_addr mismatch, the read updated cam index ba is %d, but the bsc ba_addr is %ld",(rd_cam->GetCamEntry(updated_cam_index)->sdram_addr.real_ba), ba_addr);
        sc_core::sc_time updating_time = sc_core::sc_time_stamp() + mc_cycle_time;
        ntt_store.RecordNttBsc(true,bsc_index,updating_time);
...
Scheduler::UpdateWrNttPip(BSC_INDEX bsc_index, RealBaIndex ba_addr, UpdateType update_type)
{
    ...
    if(update_type == UpdateType::NewCmdStore && !wr_cam->IsBaOrderListEmpty(ba_addr) && wr_cam->IsBaListAvail(ba_addr))
    {
        ...
        DPRINT_ASSERT(wr_cam->GetCamEntry(updated_cam_index)->sdram_addr.real_ba == ba_addr,"Wr Ntt Update:", "ba_addr mismatch, the write updated cam index ba is %d, but the bsc ba_addr is %ld",(wr_cam->GetCamEntry(updated_cam_index)->sdram_addr.real_ba), ba_addr);
        sc_core::sc_time updating_time = sc_core::sc_time_stamp() + mc_cycle_time;
        ntt_store.RecordNttBsc(false,bsc_index,updating_time);
